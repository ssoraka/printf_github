...
void	ft_summ_str_str(char *answer, char *str, int shift)
{
	...
	i = 0;
	len = ft_strlen(str);
	//while (str[i] == '0' && answer[i + shift] == '0')
	//	i++;
	while (i < len || overfl > 0)
	{
...
void	ft_decimal_part(p_list	*p, int accuracy)
{
	...
	ft_strrevers(str[1]);
	p->answer = ft_strjoin_free((p->answer), str[1], 0, 0);
	ft_str_arr_free(str);
}
...
char		*ft_ldtoa_final_e_2(char *str, int power, int accuracy)
{
	...
	ft_memset((void *)begin, '0', accuracy + 2 + ACC);
	//printf("%s\n", begin);
	begin[1] = '.';
	ft_strrevers(str);
	len = ft_strlen(str) - 1;
	...
	str[len] = '\0';
	ft_strrevers(str);
	ft_memcpy((void *)(begin + 2), str, ft_strlen(str));
	if (accuracy == 0)
		begin[1] = '\0';
	free(str);
	//printf("%s\n", begin);
...
char		*ft_ldtoa_final_e(long double ld, int accuracy)
{
	...
	//ans = ft_ldtoa_final_e_2((p->answer), power10, accuracy); //ошибка малока
	//ans = 0;
	write(1, "error1\n", 8);
